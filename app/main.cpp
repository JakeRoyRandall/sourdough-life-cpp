...
struct Options { int width = 20, height = 10, steps = 8; uint32_t seed = 2020; std::string pattern = "random", load, save, svg; bool force = false, wrap = false, loadMode = false, widthSet = false, heightSet = false, seedSet = false, patternSet = false; };

static void usage() { std::cout << "Sourdough Life — a pure toy cellular automaton\nUsage: sourdough-life [--width N] [--height N] [--steps N] [--seed N] [--pattern random|block|blinker|glider] [--load FILE] [--save FILE] [--svg FILE] [--wrap] [--force]\nLoad/save files are strict rectangular .# grids; loading takes dimensions from the file.\nFinite dead boundaries by default; --wrap enables toroidal edges.\n"; }
